    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            if cursor.fetchone()[0] == 0:
                conn.execute("INSERT INTO customers (name, email) VALUES (?, ?)", ("Ana Flask", "ana@flask.com"))
                conn.execute("INSERT INTO customers (name, email) VALUES (?, ?)", ("Bruno Flask", "bruno@flask.com"))

    def list_customers(self) -> list[Customer]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT id, name, email FROM customers")
            return [Customer(id=row[0], name=row[1], email=row[2]) for row in cursor.fetchall()]
